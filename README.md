...
| Secret 名称 | 说明 |
|---|---|
| `CF_API_TOKEN` | Cloudflare API Token（需权限：Workers Scripts Edit、Workers KV Storage Edit） |
...
- `Actions → Deploy DNSHE Assist to Cloudflare Workers → Run workflow` 手动部署

### 4. 访问

部署完成后会输出 Worker 域名（`https://dnshe-assist.<你的子域>.workers.dev`），浏览器打开即可使用。
...
# 将输出的 id 填入 wrangler.toml 的 YOUR_ASSIST_KV_ID

# 2. 设置 secrets
echo '[{"name":"...","X-API-Key":"...","X-API-Secret":"..."}]' | wrangler secret put DNSHE_ACCOUNTS
...
- DNSHE 账号凭证只存在 GitHub Secrets 和 Cloudflare Worker 环境变量中，**不会**下发到前端
- 前端页面公开可访问（无需登录），任何拿到 URL 的人都可以消耗你的助力额度，请勿公开 Worker 域名
...
│   └── index.html        # 前端页面
├── src/
│   ├── worker.js         # Worker 主入口（路由 /api/status /api/create /api/assist）
